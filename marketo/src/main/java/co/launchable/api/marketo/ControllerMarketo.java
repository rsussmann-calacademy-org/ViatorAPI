...
@Controller
@RequestMapping("/co/launchable/api/marketo/marketo")
public class ControllerMarketo {
        ...
        try {
            for (int i = 0; i < numWorkers; i++) {
                ApiSyncCustomObjectsKeysAndAttributes action =
                        new ApiSyncCustomObjectsKeysAndAttributes("co.launchable.api.marketo.marketo.syncOrders.", "Order", env);
                action.setDataSource(dataSourceContacts);
                action.setWorkerIndex(i);
                Thread t = new Thread(action);
                t.start();
    ...
    public String handleSyncMultipleLeads(WebRequest webRequest) {
        ...
        return "forward:/co.launchable.api.marketo.marketo/jobStatus?uuid=" + uuid.toString();
    }
